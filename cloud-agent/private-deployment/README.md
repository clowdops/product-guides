...
A private deployment enrols with **Central** — the ClowdOps platform at `platform.clowdops.ai` — which acts as the licensing authority. Central mints the licence and tracks how many deployments your organisation runs. It never sees your chats, credentials, or infrastructure.
...
Most customers only ever run **connected deployments**. Masters and children exist for larger, segmented, or air-gapped estates — see [Master & children](federation-master-and-children.md).
...
3. **Run the install command** on the box as root. It installs Docker, pulls the stack, enrols, and starts. *(Sysadmin — [details](install-the-appliance.md).)*
...
| [Operations](operations.md) | Both | Status lifecycle, updates, backups, decommissioning, troubleshooting |
